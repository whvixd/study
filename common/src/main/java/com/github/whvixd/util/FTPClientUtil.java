...
public class FTPClientUtil {
    ...
    public FTPClient getFTPClient(String ftpHost, int ftpPort,
        ...
        try {
            ...
            log.error("FTP的IP地址可能错误，请正确配置,e ", e);
        } catch (IOException e) {
            log.info("FTP的端口错误,请正确配置,e ", e);
        }
        return ftp;
    }
    ...
    public boolean closeFTP(FTPClient ftp) {
        ...
        try {
            ...
            log.error("FTP关闭失败,e ", e);
        } finally {
            if (ftp.isConnected()) {
                try {
                    ftp.disconnect();
                } catch (IOException ioe) {
                    log.error("FTP关闭失败,e ", ioe);
                }
            }
        }
        ...
        try {
            ...
            for (FTPFile file : files) {
                ...
                if (file.getName().equals(fileName)) {
                    ...
                    flag = ftp.retrieveFile(new String(file.getName().getBytes(UTF_8), ISO_8859_1), out);
                    // 下载成功删除文件,看项目需求
                    // ftp.deleteFile(new String(fileName.getBytes(UTF_8),ISO_8859_1));
                    out.flush();
                    out.close();
                    if (flag) {
                        log.info("下载成功");
    ...
    public boolean uploadFile(FTPClient ftp, String filePath, String ftpPath) {
        ...
        try {
            ...
            flag = ftp.storeFile(new String(tempName.getBytes(UTF_8), ISO_8859_1), in);
            if (flag) {
                log.info("上传成功");
            } else {
                log.error("上传失败");
        ...
        try {
            ...
            for (FTPFile file : files) {
                ...
                if (file.getName().equals(fileName)) {
                    ...
                    ftp.retrieveFile(new String(file.getName().getBytes(UTF_8), ISO_8859_1), out);
                    ...
                    //二进制
                    ftp.setFileType(FTPClient.BINARY_FILE_TYPE);
                    flag = ftp.storeFile(newPath + File.separator + (new String(file.getName().getBytes(UTF_8), ISO_8859_1)), in);
                    out.flush();
                    out.close();
                    in.close();
                    if (flag) {
    ...
    public boolean moveFile(FTPClient ftp, String oldPath, String newPath) {
        ...
        try {
            ...
            for (FTPFile file : files) {
                ...
                //转存目录
                flag = ftp.rename(new String(file.getName().getBytes(UTF_8), ISO_8859_1), newPath + File.separator + new String(file.getName().getBytes(UTF_8), ISO_8859_1));
                if (flag) {
    ...
    public boolean deleteByFolder(FTPClient ftp, String FtpFolder) {
        boolean flag = false;
        try {
            ftp.changeWorkingDirectory(new String(FtpFolder.getBytes(UTF_8), ISO_8859_1));
            ftp.enterLocalPassiveMode();
            FTPFile[] files = ftp.listFiles();
            for (FTPFile file : files) {
                //判断为文件则删除
                if (file.isFile()) {
                    ftp.deleteFile(new String(file.getName().getBytes(UTF_8), ISO_8859_1));
                }
                //判断是文件夹
                if (file.isDirectory()) {
            ...
            //循环完成后删除文件夹
            flag = ftp.removeDirectory(new String(FtpFolder.getBytes(UTF_8), ISO_8859_1));
            if (flag) {
    ...
    public boolean readFileByFolder(FTPClient ftp, String folderPath) {
        try {
            ftp.changeWorkingDirectory(new String(folderPath.getBytes(UTF_8), ISO_8859_1));
            //设置FTP连接模式
            ftp.enterLocalPassiveMode();
            //获取指定目录下文件文件对象集合
            FTPFile files[] = ftp.listFiles();
            ...
            for (FTPFile file : files) {
                ...
                if (file.isFile()) {
                    ...
                    if (fileName.endsWith(".txt")) {
                        in = ftp.retrieveFileStream(new String(file.getName().getBytes(UTF_8), ISO_8859_1));
